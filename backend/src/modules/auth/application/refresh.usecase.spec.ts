...
describe('RefreshUseCase', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    useCase = new RefreshUseCase(mockUserRepository, mockJwtService, mockLoginUseCase);
  });
  ...
  it('returns new tokens when refresh token is valid', async () => {
    ...
    (mockJwtService.verify as jest.Mock).mockReturnValue({ sub: 1, email: user.email });
  ...
  it('throws UnauthorizedException when user does not exist', async () => {
    (mockJwtService.verify as jest.Mock).mockReturnValue({ sub: 99, email: 'ghost@example.com' });
  ...
  it('throws UnauthorizedException when user has no stored refresh token', async () => {
    (mockJwtService.verify as jest.Mock).mockReturnValue({ sub: 1, email: 'test@example.com' });
    mockUserRepository.findById.mockResolvedValue(makeUser({ refreshTokenHash: null }));
